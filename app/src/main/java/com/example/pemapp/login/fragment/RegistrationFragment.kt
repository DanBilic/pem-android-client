...
import com.example.pemapp.onboarding.OnboardingActivity
        ...
        view.registerButton.setOnClickListener {
            if (TextUtils.isEmpty(regiEmail)) {
                ...
                val myWrite = UserData(
                    ...
                    regiPassword.toString(), "", "", listOf(),""
                )
                userDataConnection.pushWrite(myWrite)
                Snackbar.make(it, "successfully registered", Snackbar.LENGTH_LONG).show()

                val intent = Intent(getActivity(), OnboardingActivity::class.java)
                intent.putExtra("email", regiEmail.toString())
                intent.putExtra("username", regiUsername.toString())
